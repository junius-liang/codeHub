...
def dealImage(img):
    ...
    for w in range(wi):
        for h in range(hi):
            if dst[w][h] == 255:
                img[w][h] = 0
    cv2.imshow("6.去除太阳能电池片栅格部分", img)
    r1 = cv2.medianBlur(img, 5)
    cv2.imshow("7.图像的去噪处理", r1)
    blueChannel = r1[:, :, 0]
    cv2.imshow("8.拆分通道", blueChannel)
